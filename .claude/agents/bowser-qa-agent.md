...
4. **Wait 3 seconds** before closing to let any final renders/animations complete: `sleep 3`
5. **Close** the session: `playwright-cli -s=<session> close`
6. **Upload artifacts to S3** — run the upload script so screenshots persist across sessions:
   ```bash
   bash client/scripts/upload-qa-artifacts.sh <SCREENSHOTS_DIR>/<run-dir> "<story-kebab-name>"
   ```
   Capture the S3 URL printed by the script and include it in the report.
   If the script is not found or AWS is unavailable, skip silently — do not fail the run.
7. **Return** the structured report in the exact structure as detailed in the "## Report" section below.
...
**S3:** s3://eagle-eval-artifacts-695681773636-dev/qa-artifacts/browser/<timestamp>-<story-name>/ (or "not uploaded")
