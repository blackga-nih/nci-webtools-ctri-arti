# JUSTIFICATION AND APPROVAL (J&A)

## Other Than Full and Open Competition

### {{TITLE}}

### National Cancer Institute (NCI)

**Date:** {{DATE}}
**Prepared By:** {{PREPARER}}
**Estimated Value:** ${{ESTIMATED_VALUE}}
**Requisition Number:** {{REQUISITION_NUMBER}}
**Document Status:** DRAFT

---

## 1. CONTRACTING ACTIVITY

**Agency:** Department of Health and Human Services
**Operating Division:** National Institutes of Health
**Institute/Center:** National Cancer Institute
**Address:** 9609 Medical Center Drive, Bethesda, MD 20892

**Contracting Office:** {{CONTRACTING_OFFICE}}
**Contracting Officer:** {{CONTRACTING_OFFICER}}

---

## 2. DESCRIPTION OF ACTION

**Nature of Action:**
{{DESCRIPTION_OF_ACTION}}

**Type of Contract:** {{CONTRACT_TYPE}}
**Period of Performance:** {{PERIOD_OF_PERFORMANCE}}

**Proposed Contractor:**

- **Name:** {{CONTRACTOR_NAME}}
- **Address:** {{CONTRACTOR_ADDRESS}}
- **SAM UEI:** {{CONTRACTOR_UEI}}
- **Business Size:** {{BUSINESS_SIZE}}
- **Socioeconomic Status:** {{SOCIOECONOMIC_STATUS}}

---

## 3. DESCRIPTION OF SUPPLIES/SERVICES

{{DESCRIPTION_OF_SUPPLIES_SERVICES}}

**Key Requirements:**
{{KEY_REQUIREMENTS}}

**Quantity:** {{QUANTITY}}
**Estimated Value:** ${{ESTIMATED_VALUE}}

---

## 4. AUTHORITY CITED

This justification is based on the following statutory authority (check one):

☐ **FAR 6.302-1** — Only one responsible source and no other supplies or services will satisfy agency requirements

☐ **FAR 6.302-2** — Unusual and compelling urgency

☐ **FAR 6.302-3(a)(2)** — Industrial mobilization; engineering, developmental, or research capability; or expert services

☐ **FAR 6.302-4** — International agreement

☐ **FAR 6.302-5** — Authorized or required by statute (specify: {{STATUTE}})

☐ **FAR 6.302-6** — National security

☐ **FAR 6.302-7** — Public interest (Agency Head only)

**Selected Authority:** {{SELECTED_AUTHORITY}}

---

## 5. DEMONSTRATION THAT PROPOSED CONTRACTOR'S UNIQUE QUALIFICATIONS

### 5.1 Unique Qualifications

{{UNIQUE_QUALIFICATIONS}}

### 5.2 Proprietary Rights

{{PROPRIETARY_RIGHTS}}

### 5.3 Technical Capabilities

{{TECHNICAL_CAPABILITIES}}

### 5.4 Experience and Past Performance

{{EXPERIENCE_PAST_PERFORMANCE}}

---

## 6. EFFORTS TO OBTAIN COMPETITION

### 6.1 Market Research Conducted

{{MARKET_RESEARCH_CONDUCTED}}

**Sources Consulted:**
| Source | Date | Findings |
|--------|------|----------|
| SAM.gov | {{SAM_DATE}} | {{SAM_FINDINGS}} |
| GSA eLibrary | {{GSA_DATE}} | {{GSA_FINDINGS}} |
| Industry | {{INDUSTRY_DATE}} | {{INDUSTRY_FINDINGS}} |

### 6.2 Sources Contacted

| Source       | Date Contacted | Response        |
| ------------ | -------------- | --------------- |
| {{SOURCE_1}} | {{S1_DATE}}    | {{S1_RESPONSE}} |
| {{SOURCE_2}} | {{S2_DATE}}    | {{S2_RESPONSE}} |
| {{SOURCE_3}} | {{S3_DATE}}    | {{S3_RESPONSE}} |

### 6.3 Sources Sought/RFI Published

☐ Yes — Published on SAM.gov on {{SOURCES_SOUGHT_DATE}}
Responses received: {{RESPONSES_RECEIVED}}
☐ No — Rationale: {{NO_SOURCES_SOUGHT_RATIONALE}}

---

## 7. DETERMINATION OF FAIR AND REASONABLE PRICE

The Contracting Officer has determined that the anticipated price is fair and reasonable based on:

☐ Adequate price competition
☐ Prices set by law or regulation
☐ Established catalog prices
☐ Comparison to prior purchases
☐ Comparison to similar items
☐ Independent Government Cost Estimate
☐ Cost analysis
☐ Other: {{OTHER_PRICE_DETERMINATION}}

**Price Analysis Summary:**
{{PRICE_ANALYSIS_SUMMARY}}

**IGCE:** ${{IGCE_AMOUNT}}
**Proposed Price:** ${{PROPOSED_PRICE}}
**Variance:** {{VARIANCE}}%

---

## 8. DESCRIPTION OF EFFORTS TO ADDRESS ANY IMPEDIMENTS TO FUTURE COMPETITION

### 8.1 Actions to Increase Competition

{{ACTIONS_TO_INCREASE_COMPETITION}}

### 8.2 Actions to Remove Barriers

{{ACTIONS_TO_REMOVE_BARRIERS}}

### 8.3 Future Competition Strategy

{{FUTURE_COMPETITION_STRATEGY}}

---

## 9. STATEMENT OF NEED BY REQUIRING ACTIVITY

I certify that:

1. The supporting data under my cognizance is accurate and complete.
2. This requirement is a bona fide need.
3. The requirements are stated in terms that permit maximum competition.

| Role                     | Name                 | Signature | Date |
| ------------------------ | -------------------- | --------- | ---- |
| Technical Representative | {{TECH_REP_NAME}}    |           |      |
| Program Official         | {{PROGRAM_OFFICIAL}} |           |      |

---

## 10. APPROVALS

**Per FAR 6.304, approval authority based on estimated value:**

| Threshold                    | Authority                    |
| ---------------------------- | ---------------------------- |
| ≤ $750,000                   | Contracting Officer          |
| > $750,000 ≤ $15,000,000     | Competition Advocate         |
| > $15,000,000 ≤ $75,000,000  | Head of Procuring Activity   |
| > $75,000,000 ≤ $100,000,000 | Senior Procurement Executive |
| > $100,000,000               | Agency Head (non-delegable)  |

**Required Approval Level for this Action:** {{APPROVAL_LEVEL}}

### Approval Signatures

| Role                      | Name         | Signature | Date |
| ------------------------- | ------------ | --------- | ---- |
| Contracting Officer       | {{CO_NAME}}  |           |      |
| Competition Advocate      | {{CA_NAME}}  |           |      |
| {{ADDITIONAL_APPROVER_1}} | {{AA1_NAME}} |           |      |
| {{ADDITIONAL_APPROVER_2}} | {{AA2_NAME}} |           |      |

---

## ATTACHMENTS

☐ A. Market Research Report
☐ B. Sources Sought Notice and Responses
☐ C. Independent Government Cost Estimate
☐ D. Technical Evaluation
☐ E. {{OTHER_ATTACHMENT}}

---

_This document was generated by EAGLE — NCI Acquisition Assistant_
_Version: DRAFT - Subject to revision before official use_
